...
//                    $params[0] = $params[1][0];                
     ...
     * Set js
     * @param array $js
     * @param string $_MODULE module where the files will be searched
     * @param array $_DIRECTORIES_ child directory where the files will be searched 
     */
     ...
     * Set Lib
     * @param array $lib
     * @param $base_dir the base dir where the file are.
     * @param $extension file extension .js or .css
     * @param string $_MODULE module where the files will be searched
     * @param array $_DIRECTORIES_ child directory where the files will be searched
     * @param bool $_SEARCH_ROOT
     */
    public static function getLib(array $lib, $base_dir, $extension,  $_MODULE = "", $_DIRECTORIES_ = array(), $_SEARCH_ROOT = TRUE)
    {

        $define_dir = $base_dir . _DS_;
        ...
        if (count($lib)) {
            for ($i = 0; $i < count($lib); $i++) {
                if (file_exists($path_ . $lib[$i] . '.'.$extension)) {
                    switch ( $extension ){
                        case 'css':
                            echo '<link href="' . $path . $lib[$i] . '.css" type="text/css"  rel="stylesheet" media="screen,projection" >';
                            break;
                        case 'js':
                            echo '<script type="text/javascript" src="' . $path . $lib[$i] . '.' . $extension .'" ></script>';
                            break;
                        default:
                            break;
                    }
